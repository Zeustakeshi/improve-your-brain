...
import Help, {helpHtml} from "./Help.js"
...
const App = {
    ...
    loading(time){
        ...
        setTimeout(() => {
            ...
            loader.style.animation = `hiden-loader ${time*20 /100}s 1`
        },time*500)
        setTimeout(() =>{
    ...
    handleScores(currentScores, highScores){
        ...
        if (correctAnswer >= highScores.correctAnswer && rounds >= highScores.rounds){
            htmlResult = resultHtml("You Win!!", true , correctAnswer , rounds , this.highScore)
            Modal.showModal("result", htmlResult)
            this.resetLevel(true, correctAnswer, rounds)
        }else {
            htmlResult = resultHtml("You Lose!!", false , correctAnswer , rounds, this.highScore)
            Modal.showModal("result", htmlResult)
    ...
    resetLevel(isWin, correctAnswer , rounds) {
        if (isWin) {
            ...
            this.showHighScore()
            this.setConfig("highScore", this.highScore)
        }else{
            this.currentScores.correctAnswer = 0
            this.currentScores.rounds = 0
        }
